...
const Integrations: React.FC = () => {
  ...
  const filteredIntegrations = useMemo(() => {
    if (!data?.integrations_v1) return [];

    return data.integrations_v1.filter((integration: Integration) =>
      integration.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      integration.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      integration.upstream?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [data, searchTerm]);
  ...
  }, [filteredIntegrations, page, perPage]);
    ...
    return (
      <div>
        ...
        <div style={{ textAlign: 'center', padding: '2rem' }}>
          <Spinner size="lg" />
          <p style={{ marginTop: '1rem' }}>Loading integrations...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div>
        ...
        <Alert variant="danger" title="Error loading integrations">
          {error.message}
        </Alert>
      </div>
    );
